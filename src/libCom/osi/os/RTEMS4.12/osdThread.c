...
static void once(void)
{
    ...
    status = pthread_key_create(&getpthreadInfo,0);
    ...
    pthreadInfo = init_threadInfo("_main_",pcommonAttr->schedParam.sched_priority,epicsThreadGetStackSize(epicsThreadStackSmall),0,0);
...
epicsShareFunc void epicsShareAPI epicsThreadSetPriority(epicsThreadId pthreadInfo,unsigned int priority)
{
    ...
    if(!pthreadInfo->isEpicsThread) {
        fprintf(stderr,"Warning: epicsThreadSetPriority called by non epics thread\n");
        //return;
    }
    pthreadInfo->osiPriority = priority;
